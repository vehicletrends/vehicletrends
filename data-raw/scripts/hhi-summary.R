...
hhi_local_summary <- rbindlist(list(
  # By powertrain
...
hhi_local_summary[group_var == "powertrain", group_level := powertrain_labels[group_level]]
hhi_local_summary[
...
write_csv(hhi_local_summary, here('data-raw', 'hhi_local_summary.csv'))

# Export as .rda
hhi_local_summary <- as_tibble(hhi_local_summary)
usethis::use_data(hhi_local_summary, overwrite = TRUE)
...
hhi_plot(hhi_local_summary, "powertrain", "Powertrain")
hhi_plot(hhi_local_summary, "vehicle_type", "Vehicle Type")
hhi_plot(hhi_local_summary, "price_bin", "Price Bin")
